...
# ── Inyección de estilos de modo oscuro desde dark.css ──
if st.session_state.dark_mode:
    DARK_CSS_PATH = os.path.join(ROOT, "frontend", "web", "dark.css")
...
if is_admin:
    ...
    with chat_container:
        ...
        if len(st.session_state.mensajes) == 0 and not pregunta_usuario and not st.session_state.get("pregunta_activa"):
            st.markdown(
                """
                <div class="atena-hero-card">
                    <div style="margin-bottom: 14px;"><span class="atena-hero-brand" translate="no">Atena</span></div>
                    <h2 class="atena-hero-title">
                        Consultor Académico de Neuroanatomía
                    </h2>
                    <p class="atena-hero-desc">
